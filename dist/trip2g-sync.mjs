...
---`,3);if(e!==-1){let n=t.slice(4,e),o=t.slice(e+4);for(let[i,s]of Object.entries(this.meta)){let u=new RegExp(`^${i}\\s*:.*$`,"m");u.test(n)?n=n.replace(u,`${i}: ${s}`):n=n.trimEnd()+`
${i}: ${s}`}return`---
${n}
---${o}`}}return`---
...
---`,3);if(e===-1)return!1;let n=t.slice(4,e),o=this.publishField.split(",").map(i=>i.trim()).filter(i=>i);for(let i of o){let s=new RegExp(`^${i}\\s*:\\s*(.+)$`,"m"),u=n.match(s);if(u){let l=u[1].trim().toLowerCase();if(l==="true"||l==="yes"||l==="1"||l==='"true"'||l==="'true'")return!0}}return!1}};function tt(r,t,a){return r===null&&t===null||r===t?"unchanged":r!==null&&t===null?a?"server_deleted":"local_only":r===null&&t!==null?a?"local_deleted":"remote_only":a?r===a?"pull":t===a?"push":"conflict":"conflict"}async function D(r){let t=r.getSyncState(),[a,e]=await Promise.all([r.getLocalFiles(),r.getServerHashes()]),n=new Map;for(let A of e)n.set(A.path,A.hash);let o=new Map,i=t.mtimes||{},s=t.localHashes||{};for(let A of a){let x=i[A.path],f=s[A.path];if(x===A.mtime&&f)o.set(A.path,f);else{let T=await r.readFileContent(A.path),E=await r.computeHash(T);o.set(A.path,E)}}let u=new Set([...o.keys(),...n.keys()]),l=[],p=[],c=[],y=[],m=[],d=[],h=[],b=[],F=0;for(let A of u){let x=o.get(A)||null,f=n.get(A)||null,T=t.files[A]||null,E=tt(x,f,T),I={path:A,action:E,localHash:x,remoteHash:f,lastSyncedHash:T};switch(l.push(I),E){case"unchanged":F++;break;case"pull":p.push(I);break;case"push":c.push(I);break;case"conflict":y.push(I);break;case"local_only":m.push(I);break;case"remote_only":d.push(I);break;case"local_deleted":h.push(I);break;case"server_deleted":b.push(I);break}}return{classifications:l,pulls:p,pushes:c,conflicts:y,localOnly:m,remoteOnly:d,localDeleted:h,serverDeleted:b,unchanged:F}}function W(r,t){let{twoWaySync:a,hasPublishFields:e}=t,n=d=>e?e(d):!0,o=[],i=[],s=[],u=[],l=[],p=[],c=[],y=[],m=0;for(let d of r.classifications){let h=n(d.path);switch(d.action){case"unchanged":o.push(d),m++;break;case"pull":a&&h&&(o.push(d),i.push(d));break;case"push":h&&(o.push(d),s.push(d));break;case"conflict":if(a)h&&(o.push(d),u.push(d));else if(h){let b={...d,action:"push"};o.push(b),s.push(b)}break;case"local_only":h&&(o.push(d),l.push(d));break;case"remote_only":a&&(o.push(d),p.push(d));break;case"local_deleted":h&&(o.push(d),c.push(d));break;case"server_deleted":a&&(o.push(d),y.push(d));break}}return{classifications:o,pulls:i,pushes:s,conflicts:u,localOnly:l,remoteOnly:p,localDeleted:c,serverDeleted:y,unchanged:m}}async function $(r,t,a={twoWaySync:!1}){let e={pulled:0,pushed:0,conflictsResolved:0,assetsUploaded:0,assetsDownloaded:0,errors:[],updatedUrls:[],warnings:[]},n=r.getSyncState(),o=[];if(t.pulls.length>0||t.remoteOnly.length>0){let l=[...t.pulls,...t.remoteOnly],p=await et(r,l,n);e.pulled=p.count,e.errors.push(...p.errors),o.push(...p.pulledPaths)}if(o.length>0){let l=await H(r,o);e.assetsDownloaded+=l.downloaded,e.errors.push(...l.errors)}if(a.twoWaySync){let l=t.classifications.filter(p=>p.action==="unchanged"&&p.remoteHash!==null).map(p=>p.path);if(l.length>0){let p=await H(r,l);e.assetsDownloaded+=p.downloaded,e.errors.push(...p.errors)}}if(t.serverDeleted.length>0&&await nt(r,t.serverDeleted,n),t.conflicts.length>0){let l=await rt(r,t.conflicts,n);e.conflictsResolved=l.resolved,e.errors.push(...l.errors)}let i=[...t.pushes,...t.localOnly],s=[];if(i.length>0&&await r.confirmPush(i.map(p=>p.path))){let p=await at(r,i,n);e.pushed=p.count,e.errors.push(...p.errors),s=p.pushedNotes}if(t.localDeleted.length>0&&await st(r,t.localDeleted,n),s.length>0){let l=await it(r,s,a.twoWaySync);e.assetsUploaded=l.uploaded,e.assetsDownloaded=l.downloaded,e.errors.push(...l.errors)}let u=t.classifications.filter(l=>l.action==="unchanged"&&l.remoteHash!==null).map(l=>l.path);if(u.length>0){let l=await ut(r,u);e.assetsUploaded+=l.uploaded,e.errors.push(...l.errors)}if(e.pushed>0||e.assetsUploaded>0){let l=await r.commitNotes();e.updatedUrls=l.updated.map(({path:p,url:c})=>({path:p,url:c}));for(let p of l.updated)for(let c of p.warnings)e.warnings.push({path:p.path,level:c.level,message:c.message})}return await r.saveSyncState(n),e}async function et(r,t,a){if(t.length===0)return{count:0,errors:[],pulledPaths:[]};let e=t.map(c=>c.path),n=[],o=[],i=0,s=await r.fetchNoteContents(e),u=new Map(s.map(c=>[c.path,c.content])),l=t.length,p=0;for(let c of t){p++,r.onProgress({step:"pull",current:p,total:l,path:c.path});let y=u.get(c.path);if(y===void 0){n.push(`Failed to fetch: ${c.path}`);continue}try{let m=c.path.substring(0,c.path.lastIndexOf("/"));m&&await r.createFolder(m),await r.writeFile(c.path,y);let d=await r.computeHash(y);a.files[c.path]=d,i++,o.push(c.path)}catch(m){n.push(`Failed to write ${c.path}: ${m}`)}}return{count:i,errors:n,pulledPaths:o}}async function at(r,t,a){if(t.length===0)return{count:0,errors:[],pushedNotes:[],urls:[]};let e=[],n=[],o=t.length,i=0;for(let d of t){i++,r.onProgress({step:"push",current:i,total:o,path:d.path});try{let h=await r.readFileContent(d.path);n.push({path:d.path,content:h})}catch(h){e.push(`Failed to read ${d.path}: ${h}`)}}if(n.length===0)return{count:0,errors:e,pushedNotes:[],urls:[]};let s=new Set(n.map(d=>d.path)),u=r.pushBatchSize||100,l=[];for(let d=0;d<n.length;d+=u){let h=n.slice(d,d+u),b=await r.pushNotes(h,!0);l.push(...b)}let p=new Set(l.map(d=>d.path)),c=0;for(let d of n)if(p.has(d.path)){let h=await r.computeHash(d.content);a.files[d.path]=h,c++}let y=l.filter(d=>s.has(d.path)),m=y.filter(d=>typeof d.url=="string").map(d=>({path:d.path,url:d.url}));return{count:c,errors:e,pushedNotes:y,urls:m}}async function rt(r,t,a){if(t.length===0)return{resolved:0,errors:[]};let e=[],n=t.map(p=>p.path),o=await r.fetchNoteContents(n),i=new Map(o.map(p=>[p.path,p.content])),s=[];for(let p of t){let c=i.get(p.path);if(c!==void 0)try{let y=await r.readFileContent(p.path);s.push({path:p.path,localContent:y,remoteContent:c,localHash:p.localHash,remoteHash:p.remoteHash})}catch(y){console.warn(`Failed to read local file for conflict ${p.path}:`,y),e.push(`Failed to read local file for conflict: ${p.path}`)}}if(s.length===0)return{resolved:0,errors:e};let u=await r.onConflict(s),l=0;for(let p=0;p<s.length;p++){let c=s[p],y=u[p]||"skip";try{await ot(r,c,y,a),y!=="skip"&&l++}catch(m){e.push(`Failed to resolve conflict for ${c.path}: ${m}`)}}return{resolved:l,errors:e}}async function ot(r,t,a,e){switch(a){case"keep_local":await r.pushNotes([{path:t.path,content:t.localContent}],!0),e.files[t.path]=t.localHash;break;case"keep_remote":await r.writeFile(t.path,t.remoteContent),e.files[t.path]=t.remoteHash;break;case"keep_both":{let n=t.path.substring(t.path.lastIndexOf(".")),i=`${t.path.substring(0,t.path.lastIndexOf("."))} (server)${n}`;await r.writeFile(i,t.remoteContent),e.files[t.path]=t.localHash;let s=await r.computeHash(t.remoteContent);e.files[i]=s;break}case"skip":break}}async function nt(r,t,a){if(t.length===0)return;let e=t.map(o=>o.path);if(await r.onServerDeleted(e))for(let o of t)try{await r.deleteFile(o.path),delete a.files[o.path]}catch(i){console.warn(`Failed to delete file ${o.path}:`,i)}else for(let o of t)o.localHash&&(a.files[o.path]=o.localHash)}async function st(r,t,a){if(t.length===0)return;let e=t.map(n=>n.path);await r.hideNotes(e);for(let n of e)delete a.files[n]}async function it(r,t,a){console.log(`[Trip2g Sync] syncAssets called with ${t.length} notes, twoWaySync=${a}`);let e={uploaded:0,downloaded:0,conflictsResolved:0,errors:[]};if(t.length===0)return e;let n=[],o=[],i=[];for(let s of t)if(console.log(`[Trip2g Sync] Processing assets for note: ${s.path}, assets count: ${s.assets?.length??0}`),!(!s.assets||s.assets.length===0))for(let u of s.assets){let l=await r.resolveAssetPath(u.path,s.path);if(console.log(`[Trip2g Sync] Asset "${u.path}" -> localPath: ${l??"NOT FOUND"}, sha256Hash: ${u.sha256Hash??"null"}`),!l)continue;if(!u.sha256Hash||!u.absolutePath||!u.url){console.log(`[Trip2g Sync] Queuing upload: ${u.path} (no hash on server)`),n.push({noteId:s.id,notePath:s.path,asset:u,localPath:l});continue}if(await r.fileExists(l))try{let c=await r.readBinaryFile(l),y=await r.computeBinaryHash(c);if(y===u.sha256Hash)continue;i.push({path:u.path,absolutePath:l,noteId:s.id,localHash:y,remoteHash:u.sha256Hash,remoteUrl:u.url})}catch(c){e.errors.push(`Failed to read local asset ${l}: ${c}`)}else a&&o.push({asset:u,localPath:l})}if(console.log(`[Trip2g Sync] Assets to upload: ${n.length}, to download: ${o.length}, conflicts: ${i.length}`),n.length>0){let s=new Map;for(let c of n){let y=`${c.noteId}:${c.localPath}`;s.has(y)||s.set(y,c)}let u=Array.from(s.values()),l=u.length,p=0;console.log(`[Trip2g Sync] Uploading ${l} unique (note, asset) pairs`);for(let c of u){p++,console.log(`[Trip2g Sync] Uploading asset ${p}/${l}: ${c.localPath}`),r.onProgress({step:"upload_asset",current:p,total:l,path:c.asset.path});try{let y=await r.readBinaryFile(c.localPath),m=await r.computeBinaryHash(y),d=new Blob([y]),h=c.localPath.substring(c.localPath.lastIndexOf("/")+1);await r.uploadAsset({noteId:c.noteId,blob:d,fileName:h,relativePath:c.asset.path,absolutePath:c.localPath,sha256Hash:m})&&e.uploaded++}catch(y){e.errors.push(`Failed to upload asset ${c.asset.path}: ${y}`)}}}if(o.length>0){let s=o.length,u=0;for(let l of o)if(u++,r.onProgress({step:"download_asset",current:u,total:s,path:l.asset.path}),!!l.asset.url)try{let p=await r.downloadAsset(l.asset.url);if(!p){e.errors.push(`Failed to download asset ${l.asset.path}`);continue}let c=l.localPath.substring(0,l.localPath.lastIndexOf("/"));c&&await r.createFolder(c),await r.writeBinaryFile(l.localPath,p),e.downloaded++}catch(p){e.errors.push(`Failed to download asset ${l.asset.path}: ${p}`)}}if(i.length>0){let s=await lt(r,i,a);e.uploaded+=s.uploaded,e.downloaded+=s.downloaded,e.conflictsResolved=s.conflictsResolved,e.errors.push(...s.errors)}return e}async function lt(r,t,a){let e={uploaded:0,downloaded:0,conflictsResolved:0,errors:[]};if(t.length===0)return e;let n;a?n=await r.onAssetConflict(t):n=t.map(()=>"keep_local");for(let o=0;o<t.length;o++){let i=t[o],s=n[o]||"skip";try{if(s==="keep_local"){let u=await r.readBinaryFile(i.absolutePath),l=new Blob([u]),p=i.absolutePath.substring(i.absolutePath.lastIndexOf("/")+1);await r.uploadAsset({noteId:i.noteId,blob:l,fileName:p,relativePath:i.path,absolutePath:i.absolutePath,sha256Hash:i.localHash})&&(e.uploaded++,e.conflictsResolved++)}else if(s==="keep_remote"){let u=await r.downloadAsset(i.remoteUrl);u?(await r.writeBinaryFile(i.absolutePath,u),e.downloaded++,e.conflictsResolved++):e.errors.push(`Failed to download asset ${i.path}`)}}catch(u){e.errors.push(`Failed to resolve asset conflict for ${i.path}: ${u}`)}}return e}async function H(r,t){let a={downloaded:0,errors:[]};if(t.length===0)return a;let e=await r.fetchNoteAssets(t);if(e.length===0)return a;let n=new Map;for(let s of e)for(let u of s.assets){let l=u.absolutePath.replace(/^\//,"");n.has(l)||await r.fileExists(l)||n.set(l,{url:u.url,hash:u.hash})}if(n.size===0)return a;let o=n.size,i=0;for(let[s,{url:u}]of n){i++,r.onProgress({step:"download_asset",current:i,total:o,path:s});try{let l=await r.downloadAsset(u);if(!l){a.errors.push(`Failed to download asset ${s}`);continue}let p=s.substring(0,s.lastIndexOf("/"));p&&await r.createFolder(p),await r.writeBinaryFile(s,l),a.downloaded++}catch(l){a.errors.push(`Failed to download asset ${s}: ${l}`)}}return a}async function ut(r,t){let a={uploaded:0,errors:[]};if(t.length===0)return a;let e=await r.fetchNoteAssets(t);if(e.length===0)return a;let n=[];for(let s of e)for(let u of s.assets){let l=u.absolutePath?.replace(/^\//,"");if(!l&&u.id){let c=s.path.includes("/")?s.path.substring(0,s.path.lastIndexOf("/")):"",y=u.id.replace(/^\.\//,"");l=c?`${c}/${y}`:y}if(!(!l||!await r.fileExists(l)))try{let c=await r.readBinaryFile(l),y=await r.computeBinaryHash(c);if(y===u.hash)continue;n.push({noteId:s.noteId,notePath:s.path,assetPath:u.id,localPath:l,localHash:y})}catch(c){a.errors.push(`Failed to read local asset ${l}: ${c}`)}}if(n.length===0)return a;let o=n.length,i=0;for(let s of n){i++,r.onProgress({step:"upload_asset",current:i,total:o,path:s.assetPath});try{let u=await r.readBinaryFile(s.localPath),l=new Blob([u]),p=s.localPath.substring(s.localPath.lastIndexOf("/")+1);await r.uploadAsset({noteId:s.noteId,blob:l,fileName:p,relativePath:s.assetPath,absolutePath:s.localPath,sha256Hash:s.localHash})&&a.uploaded++}catch(u){a.errors.push(`Failed to upload asset ${s.assetPath}: ${u}`)}}return a}function L(){try{let r=G.join(process.cwd(),".obsidian","plugins","trip2g","data.json"),a=JSON.parse(k.readFileSync(r,"utf8"))?.syncDirs?.[0];return a?{apiUrl:a.apiUrl?`${a.apiUrl}/_system/graphql`:void 0,apiKey:a.apiKey||void 0}:{}}catch{return{}}}function pt(){let r=process.argv.slice(2),t=L(),a={folder:"",prefix:"",apiUrl:process.env.TRIP2G_ENDPOINT||process.env.ENDPOINT||t.apiUrl||"http://localhost:8081/_system/graphql",apiKey:process.env.TRIP2G_API_KEY||process.env.API_KEY||t.apiKey||"",twoWaySync:!1,verbose:!1,dryRun:!1,conflictResolution:"local",meta:{},updatedOutput:""},e=[];for(let n=0;n<r.length;n++){let o=r[n],i;if(o.includes("=")&&o.startsWith("-")){let s=o.indexOf("=");i=o.substring(s+1),o=o.substring(0,s)}switch(o){case"--api-url":case"-u":a.apiUrl=i??r[++n];break;case"--api-key":case"-k":a.apiKey=i??r[++n];break;case"--two-way":case"-2":a.twoWaySync=!0;break;case"--verbose":case"-v":a.verbose=!0;break;case"--dry-run":case"-n":a.dryRun=!0;break;case"--conflict-resolution":case"-c":{let s=i??r[++n];s==="local"||s==="remote"||s==="skip"||s==="fail"?a.conflictResolution=s:(console.error(`\u274C Invalid conflict resolution: ${s}. Use: local, remote, skip, fail`),process.exit(1));break}case"--meta":case"-m":{let s=i??r[++n];if(s&&s.includes("=")){let u=s.indexOf("="),l=s.substring(0,u),p=s.substring(u+1);a.meta[l]=p}else console.error(`\u274C Invalid --meta format: ${s}. Use: --meta key=value`),process.exit(1);break}case"--updated-output":case"-o":a.updatedOutput=i??r[++n];break;case"--help":case"-h":V(),process.exit(0);break;default:o.startsWith("-")||e.push(o)}}return e.length>=1&&(a.folder=e[0]),e.length>=2&&(a.prefix=e[1]),a}function V(){console.log(`
obsidian-sync CLI
