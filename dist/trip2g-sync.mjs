...
---`,3);if(e===-1)return!1;let o=t.slice(4,e),n=this.publishField.split(",").map(i=>i.trim()).filter(i=>i);for(let i of n){let s=new RegExp(`^${i}\\s*:\\s*(.+)$`,"m"),u=o.match(s);if(u){let l=u[1].trim().toLowerCase();if(l==="true"||l==="yes"||l==="1"||l==='"true"'||l==="'true'")return!0}}return!1}};function X(r,t,a){return r===null&&t===null||r===t?"unchanged":r!==null&&t===null?a?"server_deleted":"local_only":r===null&&t!==null?a?"local_deleted":"remote_only":a?r===a?"pull":t===a?"push":"conflict":"conflict"}async function v(r){let t=r.getSyncState(),[a,e]=await Promise.all([r.getLocalFiles(),r.getServerHashes()]),o=new Map;for(let A of e)o.set(A.path,A.hash);let n=new Map,i=t.mtimes||{},s=t.localHashes||{};for(let A of a){let x=i[A.path],f=s[A.path];if(x===A.mtime&&f)n.set(A.path,f);else{let T=await r.readFileContent(A.path),E=await r.computeHash(T);n.set(A.path,E)}}let u=new Set([...n.keys(),...o.keys()]),l=[],p=[],c=[],y=[],m=[],d=[],h=[],b=[],U=0;for(let A of u){let x=n.get(A)||null,f=o.get(A)||null,T=t.files[A]||null,E=X(x,f,T),I={path:A,action:E,localHash:x,remoteHash:f,lastSyncedHash:T};switch(l.push(I),E){case"unchanged":U++;break;case"pull":p.push(I);break;case"push":c.push(I);break;case"conflict":y.push(I);break;case"local_only":m.push(I);break;case"remote_only":d.push(I);break;case"local_deleted":h.push(I);break;case"server_deleted":b.push(I);break}}return{classifications:l,pulls:p,pushes:c,conflicts:y,localOnly:m,remoteOnly:d,localDeleted:h,serverDeleted:b,unchanged:U}}function D(r,t){let{twoWaySync:a,hasPublishFields:e}=t,o=d=>e?e(d):!0,n=[],i=[],s=[],u=[],l=[],p=[],c=[],y=[],m=0;for(let d of r.classifications){let h=o(d.path);switch(d.action){case"unchanged":n.push(d),m++;break;case"pull":a&&h&&(n.push(d),i.push(d));break;case"push":h&&(n.push(d),s.push(d));break;case"conflict":if(a)h&&(n.push(d),u.push(d));else if(h){let b={...d,action:"push"};n.push(b),s.push(b)}break;case"local_only":h&&(n.push(d),l.push(d));break;case"remote_only":a&&(n.push(d),p.push(d));break;case"local_deleted":h&&(n.push(d),c.push(d));break;case"server_deleted":a&&(n.push(d),y.push(d));break}}return{classifications:n,pulls:i,pushes:s,conflicts:u,localOnly:l,remoteOnly:p,localDeleted:c,serverDeleted:y,unchanged:m}}async function H(r,t,a={twoWaySync:!1}){let e={pulled:0,pushed:0,conflictsResolved:0,assetsUploaded:0,assetsDownloaded:0,errors:[],updatedUrls:[],warnings:[]},o=r.getSyncState(),n=[];if(t.pulls.length>0||t.remoteOnly.length>0){let l=[...t.pulls,...t.remoteOnly],p=await Z(r,l,o);e.pulled=p.count,e.errors.push(...p.errors),n.push(...p.pulledPaths)}if(n.length>0){let l=await W(r,n);e.assetsDownloaded+=l.downloaded,e.errors.push(...l.errors)}if(a.twoWaySync){let l=t.classifications.filter(p=>p.action==="unchanged"&&p.remoteHash!==null).map(p=>p.path);if(l.length>0){let p=await W(r,l);e.assetsDownloaded+=p.downloaded,e.errors.push(...p.errors)}}if(t.serverDeleted.length>0&&await rt(r,t.serverDeleted,o),t.conflicts.length>0){let l=await et(r,t.conflicts,o);e.conflictsResolved=l.resolved,e.errors.push(...l.errors)}let i=[...t.pushes,...t.localOnly],s=[];if(i.length>0&&await r.confirmPush(i.map(p=>p.path))){let p=await tt(r,i,o);e.pushed=p.count,e.errors.push(...p.errors),s=p.pushedNotes}if(t.localDeleted.length>0&&await ot(r,t.localDeleted,o),s.length>0){let l=await nt(r,s,a.twoWaySync);e.assetsUploaded=l.uploaded,e.assetsDownloaded=l.downloaded,e.errors.push(...l.errors)}let u=t.classifications.filter(l=>l.action==="unchanged"&&l.remoteHash!==null).map(l=>l.path);if(u.length>0){let l=await it(r,u);e.assetsUploaded+=l.uploaded,e.errors.push(...l.errors)}if(e.pushed>0||e.assetsUploaded>0){let l=await r.commitNotes();e.updatedUrls=l.updated.map(({path:p,url:c})=>({path:p,url:c}));for(let p of l.updated)for(let c of p.warnings)e.warnings.push({path:p.path,level:c.level,message:c.message})}return await r.saveSyncState(o),e}async function Z(r,t,a){if(t.length===0)return{count:0,errors:[],pulledPaths:[]};let e=t.map(c=>c.path),o=[],n=[],i=0,s=await r.fetchNoteContents(e),u=new Map(s.map(c=>[c.path,c.content])),l=t.length,p=0;for(let c of t){p++,r.onProgress({step:"pull",current:p,total:l,path:c.path});let y=u.get(c.path);if(y===void 0){o.push(`Failed to fetch: ${c.path}`);continue}try{let m=c.path.substring(0,c.path.lastIndexOf("/"));m&&await r.createFolder(m),await r.writeFile(c.path,y);let d=await r.computeHash(y);a.files[c.path]=d,i++,n.push(c.path)}catch(m){o.push(`Failed to write ${c.path}: ${m}`)}}return{count:i,errors:o,pulledPaths:n}}async function tt(r,t,a){if(t.length===0)return{count:0,errors:[],pushedNotes:[],urls:[]};let e=[],o=[],n=t.length,i=0;for(let d of t){i++,r.onProgress({step:"push",current:i,total:n,path:d.path});try{let h=await r.readFileContent(d.path);o.push({path:d.path,content:h})}catch(h){e.push(`Failed to read ${d.path}: ${h}`)}}if(o.length===0)return{count:0,errors:e,pushedNotes:[],urls:[]};let s=new Set(o.map(d=>d.path)),u=r.pushBatchSize||100,l=[];for(let d=0;d<o.length;d+=u){let h=o.slice(d,d+u),b=await r.pushNotes(h,!0);l.push(...b)}let p=new Set(l.map(d=>d.path)),c=0;for(let d of o)if(p.has(d.path)){let h=await r.computeHash(d.content);a.files[d.path]=h,c++}let y=l.filter(d=>s.has(d.path)),m=y.filter(d=>typeof d.url=="string").map(d=>({path:d.path,url:d.url}));return{count:c,errors:e,pushedNotes:y,urls:m}}async function et(r,t,a){if(t.length===0)return{resolved:0,errors:[]};let e=[],o=t.map(p=>p.path),n=await r.fetchNoteContents(o),i=new Map(n.map(p=>[p.path,p.content])),s=[];for(let p of t){let c=i.get(p.path);if(c!==void 0)try{let y=await r.readFileContent(p.path);s.push({path:p.path,localContent:y,remoteContent:c,localHash:p.localHash,remoteHash:p.remoteHash})}catch(y){console.warn(`Failed to read local file for conflict ${p.path}:`,y),e.push(`Failed to read local file for conflict: ${p.path}`)}}if(s.length===0)return{resolved:0,errors:e};let u=await r.onConflict(s),l=0;for(let p=0;p<s.length;p++){let c=s[p],y=u[p]||"skip";try{await at(r,c,y,a),y!=="skip"&&l++}catch(m){e.push(`Failed to resolve conflict for ${c.path}: ${m}`)}}return{resolved:l,errors:e}}async function at(r,t,a,e){switch(a){case"keep_local":await r.pushNotes([{path:t.path,content:t.localContent}],!0),e.files[t.path]=t.localHash;break;case"keep_remote":await r.writeFile(t.path,t.remoteContent),e.files[t.path]=t.remoteHash;break;case"keep_both":{let o=t.path.substring(t.path.lastIndexOf(".")),i=`${t.path.substring(0,t.path.lastIndexOf("."))} (server)${o}`;await r.writeFile(i,t.remoteContent),e.files[t.path]=t.localHash;let s=await r.computeHash(t.remoteContent);e.files[i]=s;break}case"skip":break}}async function rt(r,t,a){if(t.length===0)return;let e=t.map(n=>n.path);if(await r.onServerDeleted(e))for(let n of t)try{await r.deleteFile(n.path),delete a.files[n.path]}catch(i){console.warn(`Failed to delete file ${n.path}:`,i)}else for(let n of t)n.localHash&&(a.files[n.path]=n.localHash)}async function ot(r,t,a){if(t.length===0)return;let e=t.map(o=>o.path);await r.hideNotes(e);for(let o of e)delete a.files[o]}async function nt(r,t,a){console.log(`[Trip2g Sync] syncAssets called with ${t.length} notes, twoWaySync=${a}`);let e={uploaded:0,downloaded:0,conflictsResolved:0,errors:[]};if(t.length===0)return e;let o=[],n=[],i=[];for(let s of t)if(console.log(`[Trip2g Sync] Processing assets for note: ${s.path}, assets count: ${s.assets?.length??0}`),!(!s.assets||s.assets.length===0))for(let u of s.assets){let l=await r.resolveAssetPath(u.path,s.path);if(console.log(`[Trip2g Sync] Asset "${u.path}" -> localPath: ${l??"NOT FOUND"}, sha256Hash: ${u.sha256Hash??"null"}`),!l)continue;if(!u.sha256Hash||!u.absolutePath||!u.url){console.log(`[Trip2g Sync] Queuing upload: ${u.path} (no hash on server)`),o.push({noteId:s.id,notePath:s.path,asset:u,localPath:l});continue}if(await r.fileExists(l))try{let c=await r.readBinaryFile(l),y=await r.computeBinaryHash(c);if(y===u.sha256Hash)continue;i.push({path:u.path,absolutePath:l,noteId:s.id,localHash:y,remoteHash:u.sha256Hash,remoteUrl:u.url})}catch(c){e.errors.push(`Failed to read local asset ${l}: ${c}`)}else a&&n.push({asset:u,localPath:l})}if(console.log(`[Trip2g Sync] Assets to upload: ${o.length}, to download: ${n.length}, conflicts: ${i.length}`),o.length>0){let s=new Map;for(let c of o){let y=`${c.noteId}:${c.localPath}`;s.has(y)||s.set(y,c)}let u=Array.from(s.values()),l=u.length,p=0;console.log(`[Trip2g Sync] Uploading ${l} unique (note, asset) pairs`);for(let c of u){p++,console.log(`[Trip2g Sync] Uploading asset ${p}/${l}: ${c.localPath}`),r.onProgress({step:"upload_asset",current:p,total:l,path:c.asset.path});try{let y=await r.readBinaryFile(c.localPath),m=await r.computeBinaryHash(y),d=new Blob([y]),h=c.localPath.substring(c.localPath.lastIndexOf("/")+1);await r.uploadAsset({noteId:c.noteId,blob:d,fileName:h,relativePath:c.asset.path,absolutePath:c.localPath,sha256Hash:m})&&e.uploaded++}catch(y){e.errors.push(`Failed to upload asset ${c.asset.path}: ${y}`)}}}if(n.length>0){let s=n.length,u=0;for(let l of n)if(u++,r.onProgress({step:"download_asset",current:u,total:s,path:l.asset.path}),!!l.asset.url)try{let p=await r.downloadAsset(l.asset.url);if(!p){e.errors.push(`Failed to download asset ${l.asset.path}`);continue}let c=l.localPath.substring(0,l.localPath.lastIndexOf("/"));c&&await r.createFolder(c),await r.writeBinaryFile(l.localPath,p),e.downloaded++}catch(p){e.errors.push(`Failed to download asset ${l.asset.path}: ${p}`)}}if(i.length>0){let s=await st(r,i,a);e.uploaded+=s.uploaded,e.downloaded+=s.downloaded,e.conflictsResolved=s.conflictsResolved,e.errors.push(...s.errors)}return e}async function st(r,t,a){let e={uploaded:0,downloaded:0,conflictsResolved:0,errors:[]};if(t.length===0)return e;let o;a?o=await r.onAssetConflict(t):o=t.map(()=>"keep_local");for(let n=0;n<t.length;n++){let i=t[n],s=o[n]||"skip";try{if(s==="keep_local"){let u=await r.readBinaryFile(i.absolutePath),l=new Blob([u]),p=i.absolutePath.substring(i.absolutePath.lastIndexOf("/")+1);await r.uploadAsset({noteId:i.noteId,blob:l,fileName:p,relativePath:i.path,absolutePath:i.absolutePath,sha256Hash:i.localHash})&&(e.uploaded++,e.conflictsResolved++)}else if(s==="keep_remote"){let u=await r.downloadAsset(i.remoteUrl);u?(await r.writeBinaryFile(i.absolutePath,u),e.downloaded++,e.conflictsResolved++):e.errors.push(`Failed to download asset ${i.path}`)}}catch(u){e.errors.push(`Failed to resolve asset conflict for ${i.path}: ${u}`)}}return e}async function W(r,t){let a={downloaded:0,errors:[]};if(t.length===0)return a;let e=await r.fetchNoteAssets(t);if(e.length===0)return a;let o=new Map;for(let s of e)for(let u of s.assets){let l=u.absolutePath.replace(/^\//,"");o.has(l)||await r.fileExists(l)||o.set(l,{url:u.url,hash:u.hash})}if(o.size===0)return a;let n=o.size,i=0;for(let[s,{url:u}]of o){i++,r.onProgress({step:"download_asset",current:i,total:n,path:s});try{let l=await r.downloadAsset(u);if(!l){a.errors.push(`Failed to download asset ${s}`);continue}let p=s.substring(0,s.lastIndexOf("/"));p&&await r.createFolder(p),await r.writeBinaryFile(s,l),a.downloaded++}catch(l){a.errors.push(`Failed to download asset ${s}: ${l}`)}}return a}async function it(r,t){let a={uploaded:0,errors:[]};if(t.length===0)return a;let e=await r.fetchNoteAssets(t);if(e.length===0)return a;let o=[];for(let s of e)for(let u of s.assets){let l=u.absolutePath?.replace(/^\//,"");if(!l&&u.id){let c=s.path.includes("/")?s.path.substring(0,s.path.lastIndexOf("/")):"",y=u.id.replace(/^\.\//,"");l=c?`${c}/${y}`:y}if(!(!l||!await r.fileExists(l)))try{let c=await r.readBinaryFile(l),y=await r.computeBinaryHash(c);if(y===u.hash)continue;o.push({noteId:s.noteId,notePath:s.path,assetPath:u.id,localPath:l,localHash:y})}catch(c){a.errors.push(`Failed to read local asset ${l}: ${c}`)}}if(o.length===0)return a;let n=o.length,i=0;for(let s of o){i++,r.onProgress({step:"upload_asset",current:i,total:n,path:s.assetPath});try{let u=await r.readBinaryFile(s.localPath),l=new Blob([u]),p=s.localPath.substring(s.localPath.lastIndexOf("/")+1);await r.uploadAsset({noteId:s.noteId,blob:l,fileName:p,relativePath:s.assetPath,absolutePath:s.localPath,sha256Hash:s.localHash})&&a.uploaded++}catch(u){a.errors.push(`Failed to upload asset ${s.assetPath}: ${u}`)}}return a}function lt(){let r=process.argv.slice(2),t={folder:"",prefix:"",apiUrl:process.env.TRIP2G_ENDPOINT||process.env.ENDPOINT||"http://localhost:8081/graphql",apiKey:process.env.TRIP2G_API_KEY||process.env.API_KEY||"",twoWaySync:!1,verbose:!1,dryRun:!1,conflictResolution:"local",meta:{},updatedOutput:""},a=[];for(let e=0;e<r.length;e++){let o=r[e],n;if(o.includes("=")&&o.startsWith("-")){let i=o.indexOf("=");n=o.substring(i+1),o=o.substring(0,i)}switch(o){case"--api-url":case"-u":t.apiUrl=n??r[++e];break;case"--api-key":case"-k":t.apiKey=n??r[++e];break;case"--two-way":case"-2":t.twoWaySync=!0;break;case"--verbose":case"-v":t.verbose=!0;break;case"--dry-run":case"-n":t.dryRun=!0;break;case"--conflict-resolution":case"-c":{let i=n??r[++e];i==="local"||i==="remote"||i==="skip"||i==="fail"?t.conflictResolution=i:(console.error(`\u274C Invalid conflict resolution: ${i}. Use: local, remote, skip, fail`),process.exit(1));break}case"--meta":case"-m":{let i=n??r[++e];if(i&&i.includes("=")){let s=i.indexOf("="),u=i.substring(0,s),l=i.substring(s+1);t.meta[u]=l}else console.error(`\u274C Invalid --meta format: ${i}. Use: --meta key=value`),process.exit(1);break}case"--updated-output":case"-o":t.updatedOutput=n??r[++e];break;case"--help":case"-h":G(),process.exit(0);break;default:o.startsWith("-")||a.push(o)}}return a.length>=1&&(t.folder=a[0]),a.length>=2&&(t.prefix=a[1]),t}function G(){console.log(`
obsidian-sync CLI
...
`)}async function ut(){let r=process.env.TRIP2G_ENDPOINT||process.env.ENDPOINT||"http://localhost:8081/graphql",t=process.env.TRIP2G_API_KEY||process.env.API_KEY||"";t||(console.error("\u274C TRIP2G_API_KEY or API_KEY required"),process.exit(1));let e=await O({apiUrl:r,apiKey:t}).FetchAllWarnings(),o=[];for(let n of e.notePaths){let i=n.latestNoteView;if(i)for(let s of i.warnings??[])o.push({path:n.path,level:s.level,message:s.message,url:i.url??""})}console.log(JSON.stringify(o,null,2))}async function pt(){if(process.argv[2]==="warnings"){await ut();return}let r=lt();r.folder||(console.error("\u274C Error: --folder is required"),G(),process.exit(1)),r.apiKey||(console.error("\u274C Error: --api-key or API_KEY environment variable is required"),process.exit(1)),r.prefix&&r.twoWaySync&&(console.error("\u274C Error: prefix is not supported with --two-way sync"),process.exit(1)),r.dryRun&&console.log(`[dry-run] folder=${r.folder}${r.prefix?` prefix=${r.prefix}`:""}`);let t=new N({folder:r.folder,prefix:r.prefix,apiUrl:r.apiUrl,apiKey:r.apiKey,twoWaySync:r.twoWaySync,verbose:r.verbose,conflictResolution:r.conflictResolution,meta:r.meta});console.log(`
