...
}`,variables:{input:{file:null,noteId:parseInt(t.noteId),sha256Hash:t.sha256Hash,path:t.relativePath,absolutePath:t.absolutePath}}}),n=JSON.stringify({0:["variables.input.file"]}),o=new FormData;o.append("operations",e),o.append("map",n),o.append("0",t.blob,t.fileName);let i=await fetch(this.apiUrl,{method:"POST",headers:{"X-API-Key":this.apiKey},body:o});if(!i.ok)throw new Error(`HTTP ${i.status}: ${i.statusText}`);let l=await i.json();if(l.errors)throw new Error(l.errors[0]?.message||"Unknown GraphQL error");let p=l.data?.uploadNoteAsset;if(p?.__typename==="ErrorPayload")throw new Error(`Upload failed: ${p.message}`);return p?.uploadSkipped?this.log(`\u23E9 Asset skipped (already exists): ${t.relativePath}`):console.log(`\u2705 Asset uploaded: ${t.relativePath}`),!0}async downloadAsset(t){try{let a=await fetch(t);return a.ok?await a.arrayBuffer():(console.error(`\u274C Failed to download asset: HTTP ${a.status}`),null)}catch(a){return console.error(`\u274C Failed to download asset from ${t}: ${a}`),null}}async commitNotes(){try{let t=await this.sdk.CommitNotes();if("message"in t.commitNotes)throw new Error(`Commit failed: ${t.commitNotes.message}`);console.log("\u2705 Notes committed")}catch(t){console.error(`\u274C Failed to commit notes: ${t}`)}}async saveSyncState(t){let a=m.join(this.folder,F);t.lastSyncedAt=Date.now(),h.writeFileSync(a,JSON.stringify(t,null,2),"utf-8"),this.syncState=t}async computeBinaryHash(t){return B.createHash("sha256").update(Buffer.from(t)).digest("hex")}async resolveAssetPath(t,a){return w(this,t,a)}onProgress(t){this.verbose&&console.log(`  [${t.step}] ${t.current}/${t.total}: ${t.path??""}`)}async onConflict(t){if(this.conflictResolution==="fail"){console.error(`\u274C ${t.length} conflicts detected:`);for(let e of t)console.error(`   - ${e.path}`);throw new Error("Conflicts detected and --conflict-resolution=fail is set")}let a=this.cliToConflictResolution(this.conflictResolution);return console.log(`\u26A0\uFE0F ${t.length} conflicts detected, resolving with: ${this.conflictResolution}`),t.map(()=>a)}async onAssetConflict(t){if(this.conflictResolution==="fail"){console.error(`\u274C ${t.length} asset conflicts detected:`);for(let e of t)console.error(`   - ${e.path}`);throw new Error("Asset conflicts detected and --conflict-resolution=fail is set")}let a=this.cliToAssetConflictResolution(this.conflictResolution);return console.log(`\u26A0\uFE0F ${t.length} asset conflicts detected, resolving with: ${this.conflictResolution}`),t.map(()=>a)}cliToConflictResolution(t){switch(t){case"local":return"keep_local";case"remote":return"keep_remote";case"skip":return"skip";default:return"keep_local"}}cliToAssetConflictResolution(t){switch(t){case"local":return"keep_local";case"remote":return"keep_remote";case"skip":return"skip";default:return"keep_local"}}async onServerDeleted(t){return console.log(`\u26A0\uFE0F ${t.length} files deleted on server, keeping local copies`),!1}async confirmPush(t){return console.log(`\u{1F4E4} Pushing ${t.length} files...`),!0}hasPublishFieldInContent(t){if(!this.publishField)return!0;if(!t.startsWith("---"))return!1;let a=t.indexOf(`
---`,3);if(a===-1)return!1;let e=t.slice(4,a),n=this.publishField.split(",").map(o=>o.trim()).filter(o=>o);for(let o of n){let i=new RegExp(`^${o}\\s*:\\s*(.+)$`,"m"),l=e.match(i);if(l){let p=l[1].trim().toLowerCase();if(p==="true"||p==="yes"||p==="1"||p==='"true"'||p==="'true'")return!0}}return!1}};function W(r,t,a){return r===null&&t===null||r===t?"unchanged":r!==null&&t===null?a?"server_deleted":"local_only":r===null&&t!==null?a?"local_deleted":"remote_only":a?r===a?"pull":t===a?"push":"conflict":"conflict"}async function k(r){let t=r.getSyncState(),[a,e]=await Promise.all([r.getLocalFiles(),r.getServerHashes()]),n=new Map;for(let S of e)n.set(S.path,S.hash);let o=new Map,i=t.mtimes||{},l=t.localHashes||{};for(let S of a){let x=i[S.path],C=l[S.path];if(x===S.mtime&&C)o.set(S.path,C);else{let T=await r.readFileContent(S.path),N=await r.computeHash(T);o.set(S.path,N)}}let p=new Set([...o.keys(),...n.keys()]),s=[],u=[],c=[],g=[],y=[],d=[],A=[],b=[],R=0;for(let S of p){let x=o.get(S)||null,C=n.get(S)||null,T=t.files[S]||null,N=W(x,C,T),f={path:S,action:N,localHash:x,remoteHash:C,lastSyncedHash:T};switch(s.push(f),N){case"unchanged":R++;break;case"pull":u.push(f);break;case"push":c.push(f);break;case"conflict":g.push(f);break;case"local_only":y.push(f);break;case"remote_only":d.push(f);break;case"local_deleted":A.push(f);break;case"server_deleted":b.push(f);break}}return{classifications:s,pulls:u,pushes:c,conflicts:g,localOnly:y,remoteOnly:d,localDeleted:A,serverDeleted:b,unchanged:R}}function v(r,t){let{twoWaySync:a,hasPublishFields:e}=t,n=d=>e?e(d):!0,o=[],i=[],l=[],p=[],s=[],u=[],c=[],g=[],y=0;for(let d of r.classifications){let A=n(d.path);switch(d.action){case"unchanged":o.push(d),y++;break;case"pull":a&&A&&(o.push(d),i.push(d));break;case"push":A&&(o.push(d),l.push(d));break;case"conflict":if(a)A&&(o.push(d),p.push(d));else if(A){let b={...d,action:"push"};o.push(b),l.push(b)}break;case"local_only":A&&(o.push(d),s.push(d));break;case"remote_only":a&&(o.push(d),u.push(d));break;case"local_deleted":A&&(o.push(d),c.push(d));break;case"server_deleted":a&&(o.push(d),g.push(d));break}}return{classifications:o,pulls:i,pushes:l,conflicts:p,localOnly:s,remoteOnly:u,localDeleted:c,serverDeleted:g,unchanged:y}}async function H(r,t,a={twoWaySync:!1}){let e={pulled:0,pushed:0,conflictsResolved:0,assetsUploaded:0,assetsDownloaded:0,errors:[]},n=r.getSyncState(),o=[];if(t.pulls.length>0||t.remoteOnly.length>0){let s=[...t.pulls,...t.remoteOnly],u=await J(r,s,n);e.pulled=u.count,e.errors.push(...u.errors),o.push(...u.pulledPaths)}if(o.length>0){let s=await D(r,o);e.assetsDownloaded+=s.downloaded,e.errors.push(...s.errors)}let i=t.classifications.filter(s=>s.action==="unchanged"&&s.remoteHash!==null).map(s=>s.path);if(i.length>0){let s=await D(r,i);e.assetsDownloaded+=s.downloaded,e.errors.push(...s.errors)}if(t.serverDeleted.length>0&&await Z(r,t.serverDeleted,n),t.conflicts.length>0){let s=await Y(r,t.conflicts,n);e.conflictsResolved=s.resolved,e.errors.push(...s.errors)}let l=[...t.pushes,...t.localOnly],p=[];if(l.length>0&&await r.confirmPush(l.map(u=>u.path))){let u=await z(r,l,n);e.pushed=u.count,e.errors.push(...u.errors),p=u.pushedNotes}if(t.localDeleted.length>0&&await tt(r,t.localDeleted,n),p.length>0){let s=await et(r,p,a.twoWaySync);e.assetsUploaded=s.uploaded,e.assetsDownloaded=s.downloaded,e.errors.push(...s.errors)}return(e.pushed>0||e.assetsUploaded>0)&&await r.commitNotes(),await r.saveSyncState(n),e}async function J(r,t,a){if(t.length===0)return{count:0,errors:[],pulledPaths:[]};let e=t.map(c=>c.path),n=[],o=[],i=0,l=await r.fetchNoteContents(e),p=new Map(l.map(c=>[c.path,c.content])),s=t.length,u=0;for(let c of t){u++,r.onProgress({step:"pull",current:u,total:s,path:c.path});let g=p.get(c.path);if(g===void 0){n.push(`Failed to fetch: ${c.path}`);continue}try{let y=c.path.substring(0,c.path.lastIndexOf("/"));y&&await r.createFolder(y),await r.writeFile(c.path,g);let d=await r.computeHash(g);a.files[c.path]=d,i++,o.push(c.path)}catch(y){n.push(`Failed to write ${c.path}: ${y}`)}}return{count:i,errors:n,pulledPaths:o}}async function z(r,t,a){if(t.length===0)return{count:0,errors:[],pushedNotes:[]};let e=[],n=[],o=t.length,i=0;for(let y of t){i++,r.onProgress({step:"push",current:i,total:o,path:y.path});try{let d=await r.readFileContent(y.path);n.push({path:y.path,content:d})}catch(d){e.push(`Failed to read ${y.path}: ${d}`)}}if(n.length===0)return{count:0,errors:e,pushedNotes:[]};let l=new Set(n.map(y=>y.path)),p=r.pushBatchSize||100,s=[];for(let y=0;y<n.length;y+=p){let d=n.slice(y,y+p),A=await r.pushNotes(d,!0);s.push(...A)}let u=new Set(s.map(y=>y.path)),c=0;for(let y of n)if(u.has(y.path)){let d=await r.computeHash(y.content);a.files[y.path]=d,c++}let g=s.filter(y=>l.has(y.path));return{count:c,errors:e,pushedNotes:g}}async function Y(r,t,a){if(t.length===0)return{resolved:0,errors:[]};let e=[],n=t.map(u=>u.path),o=await r.fetchNoteContents(n),i=new Map(o.map(u=>[u.path,u.content])),l=[];for(let u of t){let c=i.get(u.path);if(c!==void 0)try{let g=await r.readFileContent(u.path);l.push({path:u.path,localContent:g,remoteContent:c,localHash:u.localHash,remoteHash:u.remoteHash})}catch(g){console.warn(`Failed to read local file for conflict ${u.path}:`,g),e.push(`Failed to read local file for conflict: ${u.path}`)}}if(l.length===0)return{resolved:0,errors:e};let p=await r.onConflict(l),s=0;for(let u=0;u<l.length;u++){let c=l[u],g=p[u]||"skip";try{await X(r,c,g,a),g!=="skip"&&s++}catch(y){e.push(`Failed to resolve conflict for ${c.path}: ${y}`)}}return{resolved:s,errors:e}}async function X(r,t,a,e){switch(a){case"keep_local":await r.pushNotes([{path:t.path,content:t.localContent}],!0),e.files[t.path]=t.localHash;break;case"keep_remote":await r.writeFile(t.path,t.remoteContent),e.files[t.path]=t.remoteHash;break;case"keep_both":{let n=t.path.substring(t.path.lastIndexOf(".")),i=`${t.path.substring(0,t.path.lastIndexOf("."))} (server)${n}`;await r.writeFile(i,t.remoteContent),e.files[t.path]=t.localHash;let l=await r.computeHash(t.remoteContent);e.files[i]=l;break}case"skip":break}}async function Z(r,t,a){if(t.length===0)return;let e=t.map(o=>o.path);if(await r.onServerDeleted(e))for(let o of t)try{await r.deleteFile(o.path),delete a.files[o.path]}catch(i){console.warn(`Failed to delete file ${o.path}:`,i)}else for(let o of t)o.localHash&&(a.files[o.path]=o.localHash)}async function tt(r,t,a){if(t.length===0)return;let e=t.map(n=>n.path);await r.hideNotes(e);for(let n of e)delete a.files[n]}async function et(r,t,a){let e={uploaded:0,downloaded:0,conflictsResolved:0,errors:[]};if(t.length===0)return e;let n=[],o=[],i=[];for(let l of t)if(!(!l.assets||l.assets.length===0))for(let p of l.assets){let s=await r.resolveAssetPath(p.path,l.path);if(!s)continue;if(!p.sha256Hash||!p.absolutePath||!p.url){n.push({noteId:l.id,notePath:l.path,asset:p,localPath:s});continue}if(await r.fileExists(s))try{let c=await r.readBinaryFile(s),g=await r.computeBinaryHash(c);if(g===p.sha256Hash)continue;i.push({path:p.path,absolutePath:s,noteId:l.id,localHash:g,remoteHash:p.sha256Hash,remoteUrl:p.url})}catch(c){e.errors.push(`Failed to read local asset ${s}: ${c}`)}else a&&o.push({asset:p,localPath:s})}if(n.length>0){let l=new Map;for(let c of n)l.has(c.localPath)||l.set(c.localPath,c);let p=Array.from(l.values()),s=p.length,u=0;for(let c of p){u++,r.onProgress({step:"upload_asset",current:u,total:s,path:c.asset.path});try{let g=await r.readBinaryFile(c.localPath),y=await r.computeBinaryHash(g),d=new Blob([g]),A=c.localPath.substring(c.localPath.lastIndexOf("/")+1);await r.uploadAsset({noteId:c.noteId,blob:d,fileName:A,relativePath:c.asset.path,absolutePath:c.localPath,sha256Hash:y})&&e.uploaded++}catch(g){e.errors.push(`Failed to upload asset ${c.asset.path}: ${g}`)}}}if(o.length>0){let l=o.length,p=0;for(let s of o)if(p++,r.onProgress({step:"download_asset",current:p,total:l,path:s.asset.path}),!!s.asset.url)try{let u=await r.downloadAsset(s.asset.url);if(!u){e.errors.push(`Failed to download asset ${s.asset.path}`);continue}let c=s.localPath.substring(0,s.localPath.lastIndexOf("/"));c&&await r.createFolder(c),await r.writeBinaryFile(s.localPath,u),e.downloaded++}catch(u){e.errors.push(`Failed to download asset ${s.asset.path}: ${u}`)}}if(i.length>0){let l=await at(r,i,a);e.uploaded+=l.uploaded,e.downloaded+=l.downloaded,e.conflictsResolved=l.conflictsResolved,e.errors.push(...l.errors)}return e}async function at(r,t,a){let e={uploaded:0,downloaded:0,conflictsResolved:0,errors:[]};if(t.length===0)return e;let n;a?n=await r.onAssetConflict(t):n=t.map(()=>"keep_local");for(let o=0;o<t.length;o++){let i=t[o],l=n[o]||"skip";try{if(l==="keep_local"){let p=await r.readBinaryFile(i.absolutePath),s=new Blob([p]),u=i.absolutePath.substring(i.absolutePath.lastIndexOf("/")+1);await r.uploadAsset({noteId:i.noteId,blob:s,fileName:u,relativePath:i.path,absolutePath:i.absolutePath,sha256Hash:i.localHash})&&(e.uploaded++,e.conflictsResolved++)}else if(l==="keep_remote"){let p=await r.downloadAsset(i.remoteUrl);p?(await r.writeBinaryFile(i.absolutePath,p),e.downloaded++,e.conflictsResolved++):e.errors.push(`Failed to download asset ${i.path}`)}}catch(p){e.errors.push(`Failed to resolve asset conflict for ${i.path}: ${p}`)}}return e}async function D(r,t){let a={downloaded:0,errors:[]};if(t.length===0)return a;let e=await r.fetchNoteAssets(t);if(e.length===0)return a;let n=new Map;for(let l of e)for(let p of l.assets){let s=p.absolutePath.replace(/^\//,"");n.has(s)||await r.fileExists(s)||n.set(s,{url:p.url,hash:p.hash})}if(n.size===0)return a;let o=n.size,i=0;for(let[l,{url:p}]of n){i++,r.onProgress({step:"download_asset",current:i,total:o,path:l});try{let s=await r.downloadAsset(p);if(!s){a.errors.push(`Failed to download asset ${l}`);continue}let u=l.substring(0,l.lastIndexOf("/"));u&&await r.createFolder(u),await r.writeBinaryFile(l,s),a.downloaded++}catch(s){a.errors.push(`Failed to download asset ${l}: ${s}`)}}return a}function rt(){let r=process.argv.slice(2),t={folder:"",apiUrl:process.env.ENDPOINT||"http://localhost:8081/graphql",apiKey:process.env.API_KEY||"",twoWaySync:!1,verbose:!1,dryRun:!1,conflictResolution:"local"};for(let a=0;a<r.length;a++){let e=r[a],n;if(e.includes("=")){let o=e.indexOf("=");n=e.substring(o+1),e=e.substring(0,o)}switch(e){case"--folder":case"-f":t.folder=n??r[++a];break;case"--api-url":case"-u":t.apiUrl=n??r[++a];break;case"--api-key":case"-k":t.apiKey=n??r[++a];break;case"--two-way":case"-2":t.twoWaySync=!0;break;case"--verbose":case"-v":t.verbose=!0;break;case"--dry-run":case"-n":t.dryRun=!0;break;case"--conflict-resolution":case"-c":{let o=n??r[++a];o==="local"||o==="remote"||o==="skip"||o==="fail"?t.conflictResolution=o:(console.error(`\u274C Invalid conflict resolution: ${o}. Use: local, remote, skip, fail`),process.exit(1));break}case"--help":case"-h":L(),process.exit(0);break;default:!t.folder&&!e.startsWith("-")&&(t.folder=e)}}return t}function L(){console.log(`
obsidian-sync CLI
